...
int main(int argc, char* argv[]) {
    ...
    for (const auto& result : results) {
        for (auto blockId : result.blockIds) {
            ...
            CompressionResult mgardxCompressed = compressDataWithMGARDX(blockData, blockDimensions[0], blockDimensions[1], blockDimensions[2], errorBound, s);

            std::cout << "Block ID: " << blockId << ", Isovalue: " << result.isovalue << std::endl;
            std::cout << "MGARD Compressed Size: " << mgardxCompressed.compressedData.size() << " bytes" << std::endl;
        }
    }
}
